...
class Model(nn.Module):
    ...
    def __init__(self, delta_t_init, q_init, bdiv, nlayers, device, dropout):
        super(Model, self).__init__()
        self.bsize = int(len(delta_t_init) / bdiv)
        # neglecting last len(q) - bsize * bdiv elements
        self.delta_t = delta_t_init[0:self.bsize * bdiv].clone().detach()
        self.q = q_init[0:self.bsize * bdiv].clone().detach()
        self.bdiv = bdiv
        ...
        self.linear1 = nn.Linear(in_features=self.bsize*nlayers,
                                out_features=self.bsize*self.bdiv)
        self.device = device

    def forward(self, tr):
        tr = tr.view(self.bdiv, 1, self.bsize)
        device = 'cpu'
        tr = tr.to(device).float()
        h0 = self.h0.to(device).float()
        c0 = self.c0.to(device).float()
        lstm = self.lstm.to(device)
        linear1 = self.linear1.to(device).float()
