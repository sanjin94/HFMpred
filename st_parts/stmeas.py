...
def app():
    ...
    with col_input1:
        ...
        start_time = st.text_input('Select/write start time (hh:mm):')
        end_date = st.date_input('Select end date:')
        end_time = st.text_input('Select/write end time (hh:mm):')
    ...
    with col_input2:
        ...
        calc_start_time = st.text_input(
            'Select/write calculation start time (hh:mm):')
        calc_end_date = st.date_input('Select calculation end date:')
        calc_end_time = st.text_input(
            'Select/write calculation end time (hh:mm):')
        filenames = os.listdir('data/raw/')
        file_name = st.selectbox('Select input file', filenames)
        avg_const = st.number_input(
            'Averaging constant:', value=10, min_value=1)
    ...
    if calculate_button:
        ...
        u_val = U.U_avg(ts, avg_const, ts_u, first_calc, start)
        avg_l = u_val.ts_average(ts, avg_const)
        calc_start, calc_end, u_optimal = u_val.u_optim(
            [avg_l[0], avg_l[1], avg_l[4], avg_l[6]], start)
