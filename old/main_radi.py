...
class Model(nn.Module):
    ...
    def __init__(self, in_=2, L1=64, L2=128, out_=1, ksize=3):
        ...
        self.lstm1 = nn.LSTM(input_size=(data.BATCH_SIZE-ksize+1),
                             hidden_size=L1, num_layers=2, dropout=0.6)
        self.h0 = torch.randn(2, out_, L1).cuda()
        self.c0 = torch.randn(2, out_, L1).cuda()
        ...
        self.linear = nn.Linear(in_features=L2, out_features=data.BATCH_SIZE)
...
def train_model(model, train_loader, validation_loader, q_train_loader,
    ...
    for epoch in range(n_epochs):
        ...
        with torch.set_grad_enabled(False):
            ...
            model.eval()
            z = model.forward(validation_loader)
            loss = criterion(z, q_validation_loader)
...
if __name__ == '__main__':
    ...
    training_loss, validation_loss = train_model(dl_model, tr, val, q_tr, q_val,
